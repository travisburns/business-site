...
// ── Scraper / Lead Generation (Pipeline backend at NEXT_PUBLIC_SCRAPER_API_URL) ──
...
async function scraperFetch<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  // Unwrap ApiResponseDto<T>: { success, data, message }
...
export interface ScrapingProgress {
  sessionId: number;
  status: number;       // 0=Pending 1=Running 2=Complete 3=Failed
  currentPage: number;
  totalPages: number;
...
export async function getScrapedLeads(source = "YellowPages", limit = 100) {
  return scraperFetch<Paginated<ScrapedLead>>(
